...
class WideResNet(nn.Module):
    ...
    def __init__(
        ...
        depth = config.ebm.depth
        widen_factor = config.ebm.widen_factors
        channels = config.data.num_channels
        dropout = config.model.dropout
        strides = config.ebm.strides
        self.leak = 0.2
        self.in_planes = 16
        self.sum_pool = config.ebm.sum_pool
        self.norm = config.model.normalization
        self.lrelu = nn.LeakyReLU(self.leak)
        ...
        self.linear = nn.Linear(nStages[3], 1)
